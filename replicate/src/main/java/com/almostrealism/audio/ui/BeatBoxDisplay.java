...
public class BeatBoxDisplay extends JPanel implements ActionListener, KeyListener, Evaluable<byte[]> {
	...
	private JavaAudioMixer mixer;
	private OutputLine mainLine;
	private boolean stop = true;
	
	private JavaAudioSample currentSample;
	private int index;
	...
	public void setMixer(JavaAudioMixer mixer) { this.mixer = mixer; }
	
	public void addRow(JavaAudioSample s) {
		if (!this.stop) return;
	...
	public byte[] evaluate(Object[] args) {
		...
		if (this.currentSample != null)
			b = this.currentSample.evaluate(new Object[0]);
