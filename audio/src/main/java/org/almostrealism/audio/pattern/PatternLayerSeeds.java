...
public class PatternLayerSeeds {
	...
	public Stream<PatternLayer> generator(double offset, double duration,
										  double bias, int scaleTraversalDepth) {
		double count = Math.max(1.0, duration / granularity);

		List<PatternLayer> layers = IntStream.range(0, (int) count)
				.mapToObj(i ->
						factory.apply(null, position + offset + i * granularity, granularity, this.bias + bias, scaleTraversalDepth, false, params).orElse(null))
