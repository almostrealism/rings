...
public class MultiCellFilterTest implements CellFeatures, TestFeatures {
	...
	@Test
	public void scaleDelayFeedback() throws IOException {
		w("src/test/resources/Snare Perc DD.wav", "src/test/resources/Snare Perc DD.wav")
				.d(i -> new Scalar(2))
				.m(f(2, i -> new ScaleFactor(0.45 * (i + 1)))::get, c(g(0.0, 0.5), g(0.5, 0.0)))
				.o(i -> new File("scale-delay-feedback-test-" + i + ".wav"))
				.sec(8).get().run();
	}
	...
	@Test
	public void filterDelayFeedback() throws IOException {
		w("src/test/resources/Snare Perc DD.wav", "src/test/resources/Snare Perc DD.wav")
				.d(i -> new Scalar(2))
				.m(fc(i -> hp(2000, 0.1)),
						c(g(0.0, 0.3), g(0.3, 0.0)))
				.o(i -> new File("filter-delay-feedback-test-" + i + ".wav"))
				.sec(8).get().run();
	}
}
