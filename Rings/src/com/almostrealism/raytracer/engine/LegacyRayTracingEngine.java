		...
		for (int i = p.x; i < (p.x + p.dx); i++) {
			for (int j = p.y; j < (p.y + p.dy); j++) {
				...
				for (int l = 0; l < p.ssHeight; l++) {
					...
					RGB color = LegacyRayTracingEngine.lightingCalculation(ray, surfaces, lights,
		...
		if (intersect != null) {
			...
			for(int i = 0; i < allLights.length; i++) {
				...
				if (LegacyRayTracingEngine.castShadows && allLights[i].castShadows &&
						LegacyRayTracingEngine.shadowCalculation(intersect.getPoint(), allSurfaces, allLights[i]))
					return new ColorSum();
				
				if (allLights[i] instanceof SurfaceLight) {
					c = LegacyRayTracingEngine.lightingCalculation(intersect, intersect.getPoint(),
...
//		}
	...
	public static ColorSum lightingCalculation(ShadableIntersection intersection, Vector point, Vector rayDirection,
		...
		for(int i = 0; i < lights.length; i++) {
			...
			ColorProducer c = LegacyRayTracingEngine.lightingCalculation(intersection, point,
	...
	public static ColorProducer lightingCalculation(ShadableIntersection intersection, Vector point, Vector rayDirection,
		...
		if (LegacyRayTracingEngine.castShadows && light.castShadows &&
				LegacyRayTracingEngine.shadowCalculation(point, allSurfaces, light))
			return new RGB(0.0, 0.0, 0.0);
		
		if (light instanceof SurfaceLight) {
			Light l[] = ((SurfaceLight)light).getSamples();
			return LegacyRayTracingEngine.lightingCalculation(intersection, point, rayDirection,
